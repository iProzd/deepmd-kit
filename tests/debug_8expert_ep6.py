#!/usr/bin/env python3
"""Debug 8-expert EP: verify detached energy backward eliminates deadlock."""
...
# Monkey-patch to count A2A calls
_a2a_count = 0
_orig_a2a = dist.all_to_all_single

def _counted_a2a(*args, **kwargs):
    global _a2a_count
    _a2a_count += 1
    return _orig_a2a(*args, **kwargs)

dist.all_to_all_single = _counted_a2a
...
def main():
    ...
    for step in range(110):
        ...
        # 1st backward: compute force with create_graph=True
        _a2a_count = 0
        ...
        # KEY FIX: separate energy backward and force backward with barrier
        # Don't use "loss = energy + force.sum()" then loss.backward()
        # because loss.backward() would mix the energy backward graph
        # (which contains _EPMoEForward nodes with A2A) and the force
        # backward graph (which has different structure per rank),
        # causing autograd topological sort to reorder _EPMoEForward
        # backward calls differently across ranks → NCCL deadlock.
        #
        # Instead:
        # 1. energy.backward(retain_graph=True) — deterministic A2A order
        #    (energy graph is identical across ranks)
        # 2. barrier — sync ranks
        # 3. force_loss.backward() — through 1st backward graph
        _a2a_count = 0
        energy.backward(retain_graph=True)
        energy_bwd_a2a = _a2a_count
        torch.cuda.synchronize()
        dist.barrier(group=ep_group)

        _a2a_count = 0
        force_loss = force.sum() * 0.01
        force_loss.backward()
        force_bwd_a2a = _a2a_count
        torch.cuda.synchronize()
        dist.barrier(group=ep_group)

        if step < 5 or step % 20 == 0:
            log(rank, f"S{step}: fwd={fwd_a2a} bwd1={bwd1_a2a} e_bwd={energy_bwd_a2a} f_bwd={force_bwd_a2a}")

#!/usr/bin/env python3
"""Debug 8-expert EP: track NCCL SeqNum before/after each phase."""
...
def main():
    ...
    # Only run 5 steps with detailed A2A tracking
    for step in range(5):
        ...
        fwd_a2a = _a2a_count
        log(rank, f"S{step}: fwd a2a={fwd_a2a}")
        torch.cuda.synchronize()
        dist.barrier(group=ep_group)
        ...
        bwd1_a2a = _a2a_count
        log(rank, f"S{step}: bwd1 a2a={bwd1_a2a}")
        torch.cuda.synchronize()
        dist.barrier(group=ep_group)
        ...
        loss.backward()
        bwd2_a2a = _a2a_count
        log(rank, f"S{step}: bwd2 a2a={bwd2_a2a}")
        torch.cuda.synchronize()
        dist.barrier(group=ep_group)
        ...
        optimizer.step()
        log(rank, f"S{step}: done")

    log(rank, "ALL 5 STEPS DONE")

#!/usr/bin/env python3
"""Debug 8-expert EP deadlock: count A2A calls per step per rank."""
...
# Count all NCCL collectives
_a2a_count = 0
_original_fn = None

def _counting_a2a(x, send_splits, recv_splits, group):
    global _a2a_count
    _a2a_count += 1
    return _original_fn(x, send_splits, recv_splits, group)

def install_counter():
    global _original_fn
    import deepmd.pt.model.network.moe_ep_ops as ops
    import deepmd.pt.model.network.moe as moe_mod
    _original_fn = ops.all_to_all_differentiable
    ops.all_to_all_differentiable = _counting_a2a
    moe_mod.all_to_all_differentiable = _counting_a2a
...
def main():
    ...
    for step in range(5):
        ...
        fwd_count = _a2a_count

        desc = result[0]
        energy = desc.sum()

        # 1st backward (create_graph=True)
        _a2a_count = 0
        force = -torch.autograd.grad(
            energy, coord_noisy, create_graph=True, retain_graph=True
        )[0]
        bwd1_count = _a2a_count

        loss = energy + force.sum() * 0.01

        # 2nd backward (loss.backward)
        _a2a_count = 0
        loss.backward()
        bwd2_count = _a2a_count

        print(f"[rank {rank}] step {step}: fwd={fwd_count}, bwd1={bwd1_count}, bwd2={bwd2_count}, total={fwd_count+bwd1_count+bwd2_count}", flush=True)
    ...
    print(f"[rank {rank}] ALL STEPS DONE", flush=True)

...
class NeighborStatOP:
    ...
    def build(
        ...
        """
        min_rr2, max_nnei, _ = self._build_graph(coord, atype, cell, pbc)
    ...
    def build_with_edge(
        ...
        return self._build_graph(coord, atype, cell, pbc)
    ...
    def _build_graph(
        ...
        """Build the graph for neighbor statistics and edge counts."""
        ...
        edge_per_frame = tf.reduce_sum(tf.cast(nnei, tf.int64), axis=(1, 2))
...
class NeighborStat(BaseNeighborStat):
    ...
    def __init__(
        ...
        with tf.Graph().as_default() as sub_graph:
            ...
            self.op = self.build()
            self.op_with_edge = self.build_with_edge()
    ...
    def build_with_edge(self) -> tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
        """Build the graph with edge counts."""
        ret = self.neighbor_stat.build_with_edge(
            self.place_holders["coord"],
            self.place_holders["type"],
            self.place_holders["box"],
            self.place_holders["pbc"],
    ...
    def _execute_with_edge(
        ...
        minrr2, max_nnei, edge_per_frame = run_sess(
            self.sub_sess, self.op_with_edge, feed_dict=feed_dict
        )

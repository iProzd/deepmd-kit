...
class Atten2Map(torch.nn.Module):
    ...
    def forward(
        ...
        if not self.ab_g2_qkvnorm:
            ...
            # nf x nloc x nh x nnei x nnei
            attnw = torch.matmul(g2q, torch.transpose(g2k, -1, -2))

        if self.has_gate and self.ab_g2_gatenorm:
            h2 = torch.nn.functional.normalize(h2, dim=-1)
        if self.has_gate and self.ab_g2_pre_gate:
        ...
        if self.has_gate and self.ab_g2_post_gate:
            # nf x nloc x nnei x nnei
            if not self.ab_g2_gatenorm:
                gate = torch.matmul(h2, torch.transpose(h2, -1, -2)) / 3.0**0.5
            else:
                gate = torch.matmul(h2, torch.transpose(h2, -1, -2))
            # nf x nloc x nh x nnei x nnei
...
class RepformerLayer(torch.nn.Module):
    ...
    def _cal_h2g2(
        ...
        if not self.smooth:
            ...
            invnnei = (1.0 / float(nnei)) * torch.ones(
                (nf, nloc, 1, 1), dtype=g2.dtype, device=g2.device
            )
        # nf x nloc x 3 x ng2
    ...
    def _cal_grrg(self, h2g2: torch.Tensor) -> torch.Tensor:
        # nf x nloc x 3 x ng2
        nf, nloc, _, ng2 = h2g2.shape
        # nf x nloc x 3 x axis
        h2g2m = torch.split(h2g2, self.axis_neuron, dim=-1)[0]
        # nf x nloc x axis x ng2
        g1_13 = torch.matmul(torch.transpose(h2g2m, -1, -2), h2g2) / (3.0**1)
        # nf x nloc x (axisxng2)
        g1_13 = g1_13.view(nf, nloc, self.axis_neuron * ng2)
        return g1_13

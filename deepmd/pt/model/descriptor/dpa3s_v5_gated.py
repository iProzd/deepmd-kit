...
"""DPA3S V5 variant: V4 + GatedMLP on 3 key paths.

Self-contained file. Copied from V4, with:
- node_edge_linear, edge_self_linear, edge_angle_linear1 replaced by GatedMLP
- All other components (symmetrization, Bessel/Fourier basis, learnable envelope,
  remaining 4 MLPLayers, res_residual) are preserved exactly as in V4.
"""
...
class RepFlowLayerV5(torch.nn.Module):
    ...
    def __init__(
        ...
        # -- Residual parameters (identical to baseline) --
        ...
        a_residual.append(
            get_residual(
                ...
                precision=precision,
                seed=child_seed(seed, 9), trainable=trainable,
            )
        )
...
# SPLIT_MARKER_3
    ...
    @classmethod
    def deserialize(cls, data: dict) -> "RepFlowLayerV5":
        data = data.copy()
...
@DescriptorBlock.register("se_repflow_v5")
class DescrptBlockRepflowV5(DescriptorBlock):
    """V5 repflow descriptor block: V4 + GatedMLP on 3 key paths.
...
# SPLIT_MARKER_5
    ...
    def forward(
        ...
        h2g2 = RepFlowLayerV5._cal_hg_dynamic(
            edge_ebd, h2, sw,
...
@BaseDescriptor.register("dpa3s_v5_gated")
class DescrptDPA3V5(BaseDescriptor, torch.nn.Module):
    r"""DPA3S V5 descriptor: V4 + GatedMLP on 3 key paths."""
    ...
    def __init__(
        ...
        self.repflows = DescrptBlockRepflowV5(
...
# SPLIT_MARKER_8
    ...
    @classmethod
    def deserialize(cls, data: dict) -> "DescrptDPA3V5":
        data = data.copy()
        ...
        obj.repflows.layers = torch.nn.ModuleList(
            [RepFlowLayerV5.deserialize(layer) for layer in repflow_layers]
        )
        return obj
    ...
    def enable_compression(self, min_nbor_dist, table_extrapolate=5,
                           ...
                           check_frequency=-1):
        raise NotImplementedError("Compression is unsupported for DPA3S V5.")

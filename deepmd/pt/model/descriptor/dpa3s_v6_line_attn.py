...
"""DPA3S V6 variant: V5 + Line Graph Attention.

Self-contained file. Copied from V5, with:
- Line graph attention (dimwise softmax) added before existing angle->edge path
- Angle features become stateful (updated across layers)
- All other components (symmetrization, Bessel/Fourier, learnable envelope,
  GatedMLP on 3 paths, remaining MLPLayers, res_residual) preserved from V5.
"""
...
class RepFlowLayerV6(torch.nn.Module):
    """A single repflow layer for V5 variant.

    Based on V4, with 3 key MLPLayers replaced by GatedMLP:
    - node_edge_linear -> node_edge_gated (edge->node message)
    - edge_self_linear -> edge_self_gated (edge self-update)
    - edge_angle_linear1 -> edge_angle_gated1 (angle->edge message)
    All other components (symmetrization, learnable envelope) preserved.
    """
    ...
    def __init__(
        ...
        # -- V5: 3 key paths replaced by GatedMLP --
        self.node_edge_gated = GatedMLP(
            self.edge_info_dim, gated_hidden_dim, n_dim,
            precision=precision, seed=child_seed(seed, 20),
        )
        self.edge_self_gated = GatedMLP(
            self.edge_info_dim, gated_hidden_dim, e_dim,
            precision=precision, seed=child_seed(seed, 21),
        )
        self.edge_angle_gated1 = GatedMLP(
            self.angle_dim, gated_hidden_dim, e_dim,
            precision=precision, seed=child_seed(seed, 22),
        )

        # -- V6 addition: Line Graph Attention --
        # edge update MLP for line graph attention
        self.line_attn_edge_mlp = GatedMLP(
            a_dim + 2 * e_dim, gated_hidden_dim, e_dim,
            precision=precision, seed=child_seed(seed, 30),
        )
        # attention weight linear (projects angle features for dimwise softmax)
        ...
        # angle update MLP for line graph attention
        self.line_attn_angle_mlp = MLPLayer(
            a_dim + 2 * e_dim, a_dim, precision=precision,
            seed=child_seed(seed, 32), trainable=trainable,
        )

        # -- V4 addition: learnable envelope --
        self.learnable_envelope = MLPLayer(
            num_edge_basis, n_dim, bias=False, precision=precision,
            seed=child_seed(seed, 10), trainable=trainable,
        )

        # -- Residual parameters --
        # 3 for node: self, sym, edge_msg (same as V5)
        ...
        # 3 for edge: self, angle_msg, V6_line_attn
        ...
        for ii in range(2):
            a_residual.append(
                get_residual(
                    ...
                    precision=precision,
                    seed=child_seed(child_seed(seed, 9), ii),
                    trainable=trainable,
                )
            )
...
# SPLIT_MARKER_2
    ...
    def forward(
        ...
        edge_rbf=None,  # V4: raw Bessel basis for learnable envelope
    ):
        ...
        # ---- 3. Node edge message (V5: GatedMLP + V4 learnable envelope) ----
        node_i = torch.index_select(
            node_ebd.reshape(-1, self.n_dim), 0, n2e_index
        )
        edge_info = torch.cat([node_i, nei_node_ebd, edge_ebd], dim=-1)
        node_edge_update = self.node_edge_gated(edge_info)

        # V4: learnable envelope modulation (multiplicative with sw)
        if edge_rbf is not None:
            envelope_weight = torch.sigmoid(self.learnable_envelope(edge_rbf))
            node_edge_update = node_edge_update * sw.unsqueeze(-1) * envelope_weight
        else:
            node_edge_update = node_edge_update * sw.unsqueeze(-1)
        ...
        # ---- 4. Edge self (V5: GatedMLP) ----
        edge_self = self.edge_self_gated(edge_info)

        # ---- V6: Line Graph Attention (before angle->edge path) ----
        edge_ik_la = torch.index_select(edge_ebd, 0, eik2a_index)
        edge_ij_la = torch.index_select(edge_ebd, 0, eij2a_index)
        line_attn_info = torch.cat([angle_ebd, edge_ik_la, edge_ij_la], dim=-1)

        # Attention weights: dimwise softmax over angles grouped by edge_ij
        attn_logits = self.line_attn_weight_linear(angle_ebd)
        # Multiply by a_sw before softmax to zero out contributions at cutoff
        attn_logits = attn_logits * a_sw.unsqueeze(-1)
        alpha = dimwise_softmax(attn_logits, eij2a_index, num_segment=n_edge)

        # Edge update from line graph attention
        ...
        # Angle update from line graph attention
        line_angle_update = self.act(self.line_attn_angle_mlp(line_attn_info))

        # Apply line graph attention residuals
        edge_ebd = edge_ebd + self.e_residual[2] * line_edge_agg
        angle_ebd = angle_ebd + self.a_residual[1] * line_angle_update

        # ---- 5. Angle info (uses updated edge_ebd and angle_ebd) ----
        ...
        # ---- 6. Edge angle message (V5: GatedMLP for linear1) ----
        edge_angle = self.edge_angle_gated1(angle_info)
...
# SPLIT_MARKER_3
    ...
    def serialize(self) -> dict:
        data = {
            ...
            "node_edge_gated": self.node_edge_gated.serialize(),
            "edge_self_gated": self.edge_self_gated.serialize(),
            "edge_angle_gated1": self.edge_angle_gated1.serialize(),
            "edge_angle_linear2": self.edge_angle_linear2.serialize(),
            "angle_self_linear": self.angle_self_linear.serialize(),
            "learnable_envelope": self.learnable_envelope.serialize(),
    ...
    @classmethod
    def deserialize(cls, data: dict) -> "RepFlowLayerV6":
        data = data.copy()
        check_version_compatibility(data.pop("@version"), 1, 1)
        data.pop("@class")
        node_self_mlp = data.pop("node_self_mlp")
        node_sym_linear = data.pop("node_sym_linear")
        node_edge_gated = data.pop("node_edge_gated")
        edge_self_gated = data.pop("edge_self_gated")
        edge_angle_gated1 = data.pop("edge_angle_gated1")
        edge_angle_linear2 = data.pop("edge_angle_linear2")
        angle_self_linear = data.pop("angle_self_linear")
        learnable_envelope = data.pop("learnable_envelope")
        ...
        obj.node_edge_gated = GatedMLP.deserialize(node_edge_gated)
        obj.edge_self_gated = GatedMLP.deserialize(edge_self_gated)
        obj.edge_angle_gated1 = GatedMLP.deserialize(edge_angle_gated1)
        ...
        obj.learnable_envelope = MLPLayer.deserialize(learnable_envelope)
...
@DescriptorBlock.register("se_repflow_v6")
class DescrptBlockRepflowV6(DescriptorBlock):
    """V6 repflow descriptor block: V5 + Line Graph Attention.
    ...
    def forward(
        ...
        for idx, ll in enumerate(self.layers):
            ...
            node_ebd, edge_ebd, angle_ebd = ll.forward(
                ...
                edge_rbf=edge_rbf,  # V4: pass raw basis for envelope
            )

        # final rotation matrix
        h2g2 = RepFlowLayerV6._cal_hg_dynamic(
            edge_ebd, h2, sw,
...
@BaseDescriptor.register("dpa3s_v6_line_attn")
class DescrptDPA3V6(BaseDescriptor, torch.nn.Module):
    r"""DPA3S V6 descriptor: V5 + Line Graph Attention."""
    ...
    def __init__(
        ...
        self.repflows = DescrptBlockRepflowV6(
...
# SPLIT_MARKER_8
    ...
    @classmethod
    def deserialize(cls, data: dict) -> "DescrptDPA3V6":
        data = data.copy()
        ...
        obj.repflows.layers = torch.nn.ModuleList(
            [RepFlowLayerV6.deserialize(layer) for layer in repflow_layers]
        )
        return obj
    ...
    def enable_compression(self, min_nbor_dist, table_extrapolate=5,
                           ...
                           check_frequency=-1):
        raise NotImplementedError("Compression is unsupported for DPA3S V6.")

...
"""DPA3 V2 variant: RBF/Chebyshev basis + Pre-RMSNorm (Pre-LN pattern).

Self-contained file. Do NOT import from dpa3s.py, repflow_s.py, or other variants.

V2 = V1 + Pre-RMSNorm applied inside each RepFlowLayer (Pre-LN pattern).
"""
...
class RepFlowLayerV2(torch.nn.Module):
    """A single repflow layer for V2 variant.

    V2 = V1 + Pre-RMSNorm (Pre-LN pattern). Adds three RMSNorm modules
    (node_norm, edge_norm, angle_norm) and normalizes inputs before each
    sub-update, while residuals are added to the ORIGINAL (non-normed) tensors.
    """
    ...
    def __init__(
        ...
        # V2 addition: Pre-RMSNorm layers.
        # Created on CPU explicitly to avoid inheriting any default-device context
        # set during testing (e.g. cuda:9999999 sentinel). The module will be
        # moved to the target device via the standard .to(device) call.
        with torch.device("cpu"):
            self.node_norm = RMSNorm(n_dim)
            self.edge_norm = RMSNorm(e_dim)
            self.angle_norm = RMSNorm(a_dim)

        self.node_self_mlp = MLPLayer(
            n_dim, n_dim, precision=precision,
            seed=child_seed(seed, 0), trainable=trainable, init=mlp_init,
        )
        self.node_sym_linear = MLPLayer(
            self.n_sym_dim, n_dim, precision=precision,
            seed=child_seed(seed, 1), trainable=trainable, init=mlp_init,
        )
        self.node_edge_linear = MLPLayer(
            self.edge_info_dim, n_dim, precision=precision,
            seed=child_seed(seed, 2), trainable=trainable, init=mlp_init,
        )
        self.edge_self_linear = MLPLayer(
            self.edge_info_dim, e_dim, precision=precision,
            seed=child_seed(seed, 3), trainable=trainable, init=mlp_init,
        )
        self.edge_angle_linear1 = MLPLayer(
            self.angle_dim, e_dim, precision=precision,
            seed=child_seed(seed, 4), trainable=trainable, init=mlp_init,
        )
        self.edge_angle_linear2 = MLPLayer(
            e_dim, e_dim, precision=precision,
            seed=child_seed(seed, 5), trainable=trainable, init=mlp_init,
        )
        self.angle_self_linear = MLPLayer(
            self.angle_dim, a_dim, precision=precision,
            seed=child_seed(seed, 6), trainable=trainable, init=mlp_init,
        )
    ...
    def forward(
        ...
        # V2: Pre-LN — normalize local node, edge, angle embeddings
        node_ebd_normed = self.node_norm(node_ebd)
        edge_ebd_normed = self.edge_norm(edge_ebd)
        angle_ebd_normed = self.angle_norm(angle_ebd)

        # nei_node_ebd: from ext tensor (ghost atoms) — do NOT norm ext tensor,
        # only gather from the normed local part where possible.
        # Ghost atoms (index >= nloc) are not normed; we only norm local atoms.
        # Implementation: gather from node_ebd_ext for ghost atoms, but use
        # node_ebd_normed for local atoms via index_select on the normed ext view.
        # We construct a normed ext tensor: local part normed, ghost part unchanged.
        ...
        # All sub-computations use normed inputs
        node_self = self.act(self.node_self_mlp(node_ebd_normed))

        sym_edge = self._symmetrization_op_dynamic(
            edge_ebd_normed, h2, sw, n2e_index, nb * nloc, nb, nloc
        )
        ...
        node_i = torch.index_select(
            node_ebd_normed.reshape(-1, self.n_dim), 0, n2e_index
        )
        edge_info = torch.cat([node_i, nei_node_ebd, edge_ebd_normed], dim=-1)
        node_edge_update = (
        ...
        # Residuals add to ORIGINAL (non-normed) tensors
        n_updated = (
            node_ebd
        ...
        node_for_angle = torch.index_select(
            node_ebd_normed.reshape(-1, self.n_dim), 0, n2a_index
        )
        ...
        angle_info = torch.cat(
            [angle_ebd_normed, node_for_angle, edge_ik, edge_ij], dim=-1
        )
        ...
        # Residual adds to ORIGINAL (non-normed) tensor
    ...
    def serialize(self):
        data = {
            ...
            "node_norm": self.node_norm.serialize(),
            "edge_norm": self.edge_norm.serialize(),
            "angle_norm": self.angle_norm.serialize(),
            "@variables": {
    ...
    def deserialize(cls, data):
        ...
        node_norm = data.pop("node_norm")
        edge_norm = data.pop("edge_norm")
        angle_norm = data.pop("angle_norm")
        ...
        # Deserialize RMSNorm on CPU to avoid inheriting any default-device context
        with torch.device("cpu"):
            obj.node_norm = RMSNorm.deserialize(node_norm)
            obj.edge_norm = RMSNorm.deserialize(edge_norm)
            obj.angle_norm = RMSNorm.deserialize(angle_norm)
...
@DescriptorBlock.register("se_repflow_v2")
class DescrptBlockRepflowV2(DescriptorBlock):
    """V2 repflow descriptor block with RBF/Chebyshev basis embedding + Pre-RMSNorm."""
    ...
    def __init__(
        ...
        # V2 (same as V1): basis expansion modules
        ...
        # V2 (same as V1): input dim is num_basis instead of 1
    ...
    def forward(
        ...
        # V2 (same as V1): use basis expansion instead of raw scalar
        ...
        # V2 (same as V1): basis expansion -> MLP embedding
        ...
        h2g2 = RepFlowLayerV2._cal_hg_dynamic(
            edge_ebd, h2, sw,
            owner=edge_index[0],
...
# -- imports only needed for DescrptDPA3V2 --
...
@BaseDescriptor.register("dpa3s_v2_rbf_norm")
class DescrptDPA3V2(BaseDescriptor, torch.nn.Module):
    r"""DPA3 V2 descriptor: RBF/Chebyshev basis + Pre-RMSNorm (Pre-LN pattern)."""
    ...
    def __init__(
        ...
        self.repflows = DescrptBlockRepflowV2(
            e_rcut=e_rcut,
    ...
    def serialize(self) -> dict:
        ...
        data = {
            ...
            # type_embedding serialized conditionally below
            "repflow_variable": {
    ...
    @classmethod
    def deserialize(cls, data: dict) -> "DescrptDPA3V2":
        data = data.copy()
        ...
        obj.repflows.layers = torch.nn.ModuleList(
            [RepFlowLayerV2.deserialize(layer) for layer in repflow_layers]
        )
        return obj
    ...
    def enable_compression(self, min_nbor_dist, table_extrapolate=5,
                           ...
                           check_frequency=-1):
        raise NotImplementedError("Compression is unsupported for DPA3 V2.")

...
"""DPA3S V4 variant: Bessel basis + Fourier angle + Learnable Envelope.

Self-contained file. Copied from dpa3s.py + repflow_s.py baseline, with:
- BesselExpansion replacing distance-based edge init
- FourierExpansion replacing cosine-based angle init
- Learnable envelope added to edge->node aggregation (multiplicative with sw)

All other components (symmetrization, 7 MLPLayers, res_residual) are preserved
...
class RepFlowLayerV4(torch.nn.Module):
    """A single repflow layer for V4 variant.

    Identical to RepFlowLayerS (baseline) except:
    - forward() accepts extra `edge_rbf` argument for learnable envelope
    - edge->node message is modulated by sigmoid(learnable_envelope(edge_rbf))
    """
    ...
    def __init__(
        ...
        # -- 7 Linear layers (identical to baseline) --
...
# SPLIT_MARKER_2
    ...
    def forward(
        ...
        # ---- 3. Node edge message (with V4 learnable envelope) ----
...
# SPLIT_MARKER_3
    ...
    @classmethod
    def deserialize(cls, data: dict) -> "RepFlowLayerV4":
        data = data.copy()
...
@DescriptorBlock.register("se_repflow_v4")
class DescrptBlockRepflowV4(DescriptorBlock):
    """V4 repflow descriptor block: Bessel basis + Fourier angle + learnable envelope.

    Identical to DescrptBlockRepflowsS (baseline) except for embedding layers
    and passing edge_rbf to each layer for learnable envelope modulation.
    """
...
# SPLIT_MARKER_5
    ...
    def forward(
        ...
        h2g2 = RepFlowLayerV4._cal_hg_dynamic(
            edge_ebd, h2, sw,
...
@BaseDescriptor.register("dpa3s_v4_bessel")
class DescrptDPA3V4(BaseDescriptor, torch.nn.Module):
    r"""DPA3S V4 descriptor: Bessel basis + Fourier angle + learnable envelope."""
    ...
    def __init__(
        ...
        self.repflows = DescrptBlockRepflowV4(
...
# SPLIT_MARKER_8
    ...
    @classmethod
    def deserialize(cls, data: dict) -> "DescrptDPA3V4":
        data = data.copy()
        ...
        obj.repflows.layers = torch.nn.ModuleList(
            [RepFlowLayerV4.deserialize(layer) for layer in repflow_layers]
        )
        return obj
    ...
    def enable_compression(self, min_nbor_dist, table_extrapolate=5,
                           ...
                           check_frequency=-1):
        raise NotImplementedError("Compression is unsupported for DPA3S V4.")

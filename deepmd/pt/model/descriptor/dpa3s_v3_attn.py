...
"""DPA3 V3 variant: RBF + RMSNorm + edge-gated attention + SwiGLU FFN.

Self-contained file. Do NOT import from dpa3s.py, repflow_s.py, or other variants.

V3 is a completely new architecture:
- RBF/Chebyshev basis embedding (same as V1/V2)
- No symmetrization inside layers
- No angle state across layers (angle_ebd_init is reused every layer)
- Node update: edge-gated attention → SwiGLU FFN (2 serial steps)
- Edge update: angle → edge only (1 step)
- RMSNorm (Post-LN applied to residual output)
"""
...
class RepFlowLayerV3(torch.nn.Module):
    """A single repflow layer for V3 variant.

    New architecture with 3 serial steps per layer:
      Step 1: Angle → Edge  (aggregate angle info into edge, then RMSNorm)
      Step 2: Edge-gated Attention → Node  (sigmoid gate * sw * value, then RMSNorm)
      Step 3: SwiGLU FFN → Node  (then RMSNorm)

    No symmetrization inside this layer. angle_ebd is stateless (returned unchanged).
    """
    ...
    def __init__(
        ...
        with torch.device("cpu"):
            self.node_attn_norm = RMSNorm(n_dim)
        self.n_residual_attn = get_residual(
            n_dim, 0.1, "const",
        ...
        self.n_residual_ffn = get_residual(
            n_dim, 0.1, "const",
    ...
    def forward(
        ...
        _, eij2a_index, eik2a_index = angle_index[0], angle_index[1], angle_index[2]
        ...
        # angle_ebd is stateless: return the original (unchanged)
    ...
    def serialize(self) -> dict:
        return {
            ...
            "value_linear": self.value_linear.serialize(),
            "edge_value_linear": self.edge_value_linear.serialize(),
            "node_attn_norm": self.node_attn_norm.serialize(),
            "ffn": self.ffn.serialize(),
            "node_ffn_norm": self.node_ffn_norm.serialize(),
            "@variables": {
                "e_residual": to_numpy_array(self.e_residual),
                "n_residual_attn": to_numpy_array(self.n_residual_attn),
                "n_residual_ffn": to_numpy_array(self.n_residual_ffn),
            },
        }
    ...
    def deserialize(cls, data: dict) -> "RepFlowLayerV3":
        ...
        value_linear = data.pop("value_linear")
        edge_value_linear = data.pop("edge_value_linear")
        node_attn_norm = data.pop("node_attn_norm")
        ffn = data.pop("ffn")
        node_ffn_norm = data.pop("node_ffn_norm")
        variables = data.pop("@variables", {})
        e_residual = variables.get("e_residual")
        n_residual_attn = variables.get("n_residual_attn")
        n_residual_ffn = variables.get("n_residual_ffn")

        obj = cls(**data)
        obj.angle_mlp = MLPLayer.deserialize(angle_mlp)
        obj.gate_linear = MLPLayer.deserialize(gate_linear)
        obj.value_linear = MLPLayer.deserialize(value_linear)
        obj.edge_value_linear = MLPLayer.deserialize(edge_value_linear)
        obj.ffn = SwiGLUFFN.deserialize(ffn)
        with torch.device("cpu"):
            obj.edge_norm = RMSNorm.deserialize(edge_norm)
            obj.node_attn_norm = RMSNorm.deserialize(node_attn_norm)
            obj.node_ffn_norm = RMSNorm.deserialize(node_ffn_norm)
        if e_residual is not None:
            obj.e_residual.data = to_torch_tensor(e_residual)
        if n_residual_attn is not None:
            obj.n_residual_attn.data = to_torch_tensor(n_residual_attn)
        if n_residual_ffn is not None:
            obj.n_residual_ffn.data = to_torch_tensor(n_residual_ffn)
        return obj


@DescriptorBlock.register("se_repflow_v3")
class DescrptBlockRepflowV3(DescriptorBlock):
    """V3 repflow descriptor block.

    Uses RBF/Chebyshev basis embedding, edge-gated attention, SwiGLU FFN.
    angle_ebd_init is fixed and reused every layer (stateless angle).
    rot_mat computed once after all layers using final edge_ebd.
    """
    ...
    def __init__(
        ...
        # Basis expansion modules (same as V1/V2)
        self.edge_basis = GaussianRBF(num_basis=num_edge_basis, rcut=self.e_rcut)
    ...
    def _cal_hg_dynamic(
        ...
        """Compute h2g2 = sum_j(h2_ij * edge_ebd_ij * sw_ij) for rotation matrix."""
    ...
    def forward(
        ...
        # Basis expansion for initial embeddings
        ...
        # Initial embeddings using basis expansion
        ...
        # V3: angle_ebd_init is fixed — reused every layer (stateless)
        angle_ebd_init = self.angle_embd(self.angle_basis(cosine_flat))
        ...
        for idx, ll in enumerate(self.layers):
            ...
            # V3: pass angle_ebd_init every layer (not the returned angle_ebd)
            node_ebd, edge_ebd, _ = ll.forward(
                node_ebd_ext, edge_ebd, h2, angle_ebd_init,
                nlist, nlist_mask, sw, a_nlist, a_nlist_mask, a_sw,
                edge_index=edge_index, angle_index=angle_index,
            )

        # Compute rot_mat once after all layers using final edge_ebd
...
# -- imports only needed for DescrptDPA3V3 --
...
@BaseDescriptor.register("dpa3s_v3_attn")
class DescrptDPA3V3(BaseDescriptor, torch.nn.Module):
    r"""DPA3 V3 descriptor: edge-gated attention + SwiGLU FFN + RBF + RMSNorm."""
    ...
    def __init__(
        ...
        self.repflows = DescrptBlockRepflowV3(
            e_rcut=e_rcut,
    ...
    def deserialize(cls, data: dict) -> "DescrptDPA3V3":
        ...
        obj.repflows.layers = torch.nn.ModuleList(
            [RepFlowLayerV3.deserialize(layer) for layer in repflow_layers]
        )
        return obj
    ...
    def enable_compression(self, min_nbor_dist, table_extrapolate=5,
                           ...
                           check_frequency=-1):
        raise NotImplementedError("Compression is unsupported for DPA3 V3.")

...
class EnergyFittingNet(InvarFitting):
            ...
            self.edge_embed = NetworkCollection(
                ...
                networks=[
                    FittingNet(
                        ...
                        self.precision,
                        bias_out=True,
                        seed=child_seed(child_seed(self.seed, 3), 0),
                    )
                ],
            )
        else:
    ...
    def forward(
        ...
        result_energy = out[self.var_name]

        if self.add_edge_readout and g2 is not None and self.edge_embed is not None:
            nf, nloc = atype.shape[:2]
            norm_e_fact = self.norm_fact[0]
            if edge_index is not None:
                # dynamic sel mode: g2 is n_edge x e_dim
                edge_feature = g2.to(self.prec)
                edge_atomic_contrib = self.edge_embed.networks[0](edge_feature)
                # edge_index[0] is n2e_index (owner)
                if sw is not None:
                    # sw is n_edge
                    edge_atomic_contrib = edge_atomic_contrib * sw.unsqueeze(-1)
                # aggregate edge contributions to atoms
                ...
                if sw is not None:
                    ...
                    edge_atomic_contrib = edge_atomic_contrib * sw.unsqueeze(-1)
                # sum over neighbors
                edge_energy = edge_atomic_contrib.sum(dim=-2)
                # edge_energy: nf x nloc x 1

            result_energy = result_energy + edge_energy / norm_e_fact

        result = {self.var_name: result_energy.to(env.GLOBAL_PT_FLOAT_PRECISION)}
        if "middle_output" in out:
            result["middle_output"] = out["middle_output"].to(
    ...
    def deserialize(cls, data: dict) -> "GeneralFitting":
        ...
        if edge_nets_data is not None and obj.add_edge_readout:
            obj.edge_embed = NetworkCollection.deserialize(edge_nets_data)
        return obj
    ...
    def serialize(self) -> dict:
        ...
        if self.add_edge_readout and self.edge_embed is not None:
            dd["edge_nets"] = self.edge_embed.serialize()
        return dd

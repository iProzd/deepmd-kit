...
class EnergyStdBindLoss(TaskLoss):
    def __init__(
        ...
        **kwargs
        ...
        self.has_bind = (start_pref_bind != 0.0 and limit_pref_bind != 0.0) or inference
        ...
        self.start_pref_bind = start_pref_bind
        self.limit_pref_bind = limit_pref_bind
    ...
    def forward(self, input_dict, model, label, natoms, learning_rate, mae=False):
        ...
        def get_loss_single(input_dict_, natoms_, label_):
            model_pred = model(**input_dict_)
            ...
            pref_ae = (
                self.limit_pref_ae + (self.start_pref_ae - self.limit_pref_ae) * coef
            )
            pref_pf = (
                self.limit_pref_pf + (self.start_pref_pf - self.limit_pref_pf) * coef
            )
            pref_gf = (
                self.limit_pref_gf + (self.start_pref_gf - self.limit_pref_gf) * coef
            )
            loss = torch.zeros(
            ...
            atom_norm = 1.0 / natoms_
            if self.has_e and "energy" in model_pred and "energy" in label_:
                energy_pred = model_pred["energy"]
                energy_label = label_["energy"]
            ...
            if (
                ...
                find_force = label_.get("find_force", 0.0)
                pref_f = pref_f * find_force
                force_pred = model_pred["force"]
                force_label = label_["force"]
                ...
                if self.has_f:
                    if not self.use_l1_all:
                        ...
                        l1_force_loss = F.l1_loss(
                            force_label, force_pred, reduction="none"
                        )
                ...
                if self.has_pf and "atom_pref" in label_:
                    atom_pref = label_["atom_pref"]
                    find_atom_pref = label_.get("find_atom_pref", 0.0)
                ...
                if self.has_gf and "drdq" in label_:
                    drdq = label_["drdq"]
                    find_drdq = label_.get("find_drdq", 0.0)
                    pref_gf = pref_gf * find_drdq
                    force_reshape_nframes = force_pred.reshape(-1, natoms_ * 3)
                    force_label_reshape_nframes = force_label.reshape(-1, natoms_ * 3)
                    drdq_reshape = drdq.reshape(
                        -1, natoms_ * 3, self.numb_generalized_coord
                    )
            ...
            if self.has_v and "virial" in model_pred and "virial" in label_:
                find_virial = label_.get("find_virial", 0.0)
                pref_v = pref_v * find_virial
                diff_v = label_["virial"] - model_pred["virial"].reshape(-1, 9)
            ...
            if self.has_ae and "atom_energy" in model_pred and "atom_ener" in label_:
                atom_ener = model_pred["atom_energy"]
                atom_ener_label = label_["atom_ener"]
                find_atom_ener = label_.get("find_atom_ener", 0.0)
        ...
        model_pred_dict = {}
        loss_all = torch.zeros(
            1, dtype=env.GLOBAL_PT_FLOAT_PRECISION, device=env.DEVICE
        )[0]
        more_loss_dict = {}
        for kk in input_dict:
            natoms_i = input_dict[kk]["atype"].shape[-1]
            model_pred_, loss_, more_loss_ = get_loss_single(
                input_dict[kk], natoms_i, label[kk]
            )
            model_pred_dict[kk] = model_pred_
            loss_all += loss_
            for kkk in more_loss_:
                more_loss_dict[kkk + "_" + kk] = more_loss_[kkk]
        coef = learning_rate / self.starter_learning_rate
        pref_bind = (
            self.limit_pref_bind + (self.start_pref_bind - self.limit_pref_bind) * coef
        )
        atom_norm = 1.0 / input_dict["complex"]["atype"].shape[-1]
        if self.has_bind:
            energy_pred_ligand = model_pred_dict["ligand"]["energy"]
            energy_label_ligand = label["ligand"]["energy"]
            energy_pred_pocket = model_pred_dict["pocket"]["energy"]
            energy_label_pocket = label["pocket"]["energy"]
            energy_pred_complex = model_pred_dict["complex"]["energy"]
            energy_label_complex = label["complex"]["energy"]

            bind_pred = energy_pred_complex - energy_pred_ligand - energy_pred_pocket
            bind_label = (
                energy_label_complex - energy_label_ligand - energy_label_pocket
            )

            find_bind = 1.0
            pref_bind = pref_bind * find_bind
            l2_bind_loss = torch.mean(torch.square(bind_pred - bind_label))
            if not self.inference:

...
"""Unit tests for DPA3S V8 (dpa3s_v8_next) descriptor."""
...
from deepmd.pt.model.descriptor.dpa3s_v8_next import (
    DescrptDPA3Next,
)
...
class TestDescrptDPA3V8(unittest.TestCase, TestCaseSingleFrameWithNlist):
    def setUp(self) -> None:
    ...
    def test_param_count(self) -> None:
        ...
        dd = DescrptDPA3Next(
            ntypes=2, n_dim=128, e_dim=128, a_dim=64, nlayers=6,
            e_rcut=6.0, e_rcut_smth=5.0, e_sel=120,
            a_rcut=4.0, a_rcut_smth=3.5, a_sel=40,
            num_radial=12, num_angular=8,
            precision="float32", type_map=["O", "H"],
        )
        ...
        print(f"\n=== DPA3S V8 (DPA3-Next) Parameter Count ===")
        print(f"Total:       {total:>12,}")
        print(f"  Repflows:  {repflow_total:>12,}")
        print(f"  TypeEmbed: {tebd_total:>12,}")


class TestDPA3V8Model(unittest.TestCase):
    ...
    def _make_water_system():
        coord = torch.tensor(
            [[[1.0, 1.0, 1.0], [1.0, 1.96, 1.0], [1.0, 1.0, 1.96]]],
    ...
    def test_rotation_invariance(self) -> None:
        ...
        np.testing.assert_allclose(e0, e1, atol=1e-8, err_msg="Rotation invariance failed")

    def test_translation_invariance(self) -> None:
        coord, atype, box = self._make_water_system()
        result0 = self.model.forward_common(coord, atype, box=box)
        e0 = result0["energy"].detach().cpu().numpy()
        shift = torch.tensor([[[3.7, -2.1, 5.3]]], dtype=torch.float64, device=env.DEVICE)
        coord_shift = coord + shift
        box_diag = torch.tensor([[10.0, 10.0, 10.0]], dtype=torch.float64, device=env.DEVICE).unsqueeze(1)
        ...
        e1 = result1["energy"].detach().cpu().numpy()
        np.testing.assert_allclose(e0, e1, atol=1e-8, err_msg="Translation invariance failed")

    def test_force_finite_difference(self) -> None:
        """Validate forces = -dE/dR via finite difference (smoothness test)."""
        coord, atype, box = self._make_water_system()
        natom = coord.shape[1]
        eps = 1e-5

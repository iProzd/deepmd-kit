...
"""Unit tests for DPA3S V6 (dpa3s_v6_line_attn) descriptor."""
...
from deepmd.pt.model.descriptor.dpa3s_v6_line_attn import (
    DescrptDPA3V6,
)
...
class TestDescrptDPA3V6(unittest.TestCase, TestCaseSingleFrameWithNlist):
    def setUp(self) -> None:
    ...
    def test_forward_and_serialize(self) -> None:
        ...
        dd1 = DescrptDPA3V6.deserialize(dd0.serialize())
    ...
    def test_param_count(self) -> None:
        ...
        dd = DescrptDPA3V6(
            ntypes=2, n_dim=128, e_dim=64, a_dim=32, nlayers=6,
            e_rcut=6.0, e_rcut_smth=5.0, e_sel=120,
            a_rcut=4.0, a_rcut_smth=3.5, a_sel=40,
            num_edge_basis=7, num_angle_basis=7,
...
class TestDPA3V6Model(unittest.TestCase):
    ...
    def test_translation_invariance(self) -> None:
        ...
        shift = torch.tensor(
            [[[3.7, -2.1, 5.3]]], dtype=torch.float64, device=env.DEVICE,
        )
        coord_shift = coord + shift
        box_diag = torch.tensor(
            [[10.0, 10.0, 10.0]], dtype=torch.float64, device=env.DEVICE,
        ).unsqueeze(1)

 ...
 public:
  ...
  static torch::autograd::variable_list forward_t(
    ...
    auto options = torch::TensorOptions()
                       ...
                       .device(x_tensor.device());
    torch::Tensor y_tensor = torch::empty_like(x_tensor);
    torch::Tensor y_save_tensor = torch::empty_like(x_tensor);
    int64_t tensor_size = x_tensor.numel();
    // get the device
    std::string device;
    GetTensorDevice(x_tensor, device);
    // flat the tensors
    FPTYPE* y = y_tensor.view({-1}).data_ptr<FPTYPE>();
    FPTYPE* y_save = y_save_tensor.view({-1}).data_ptr<FPTYPE>();
    const FPTYPE* dy = dy_tensor.view({-1}).data_ptr<FPTYPE>();
    const FPTYPE* x = x_tensor.view({-1}).data_ptr<FPTYPE>();
    // compute
    if (device == "GPU") {
#if GOOGLE_CUDA || TENSORFLOW_USE_ROCM
      deepmd::thsilu_grad_gpu(y, y_save, x, dy, tensor_size, w, a);
#endif  // GOOGLE_CUDA || TENSORFLOW_USE_ROCM
    } else if (device == "CPU") {
      deepmd::thsilu_grad_cpu(y, y_save, x, dy, tensor_size, w, a);
    }
    // save data
    ctx->save_for_backward({x_tensor, y_save_tensor, dy_tensor});
    // save w, a, b
    ctx->saved_data["w"] = w;
    ctx->saved_data["a"] = a;
    return {y_tensor};
  ...
  static torch::autograd::variable_list backward_t(
    ...
    torch::autograd::variable_list saved_variables = ctx->get_saved_variables();
    torch::Tensor x_tensor = saved_variables[0];
    torch::Tensor y_save_tensor = saved_variables[1];
    ...
    return {ThsiluGradGradOp::apply(x_tensor, dy_tensor, dy2_tensor, w, a)[0],
            y_save_tensor * grad_output, at::Tensor(), at::Tensor()};
  }
};

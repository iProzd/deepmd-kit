...
void DeepSpinPT::compute(ENERGYVTYPE& ener,
  ...
  if (ago == 0) {
    ...
    if (do_message_passing) {
      if (has_null_atoms) {
        build_comm_dict_with_virtual_atoms(comm_dict, lmp_list, fwd_map,
                                           remapped_sendlist,
                                           remapped_sendlist_ptrs,
                                           remapped_sendnum, remapped_recvnum);
      } else {
      ...
      auto int32_option =
          torch::TensorOptions().device(torch::kCPU).dtype(torch::kInt32);
      comm_dict.insert_or_assign("has_spin",
                                 torch::tensor({1}, int32_option));
    }
    if (lmp_list.mapping) {

# LAMMPS example for SeZM

This directory contains a minimal end-to-end pipeline for running a
SeZM model in LAMMPS via `pair_style deepmd`.

## Files

| File            | Description                                                                                                               |
| --------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `input.json`    | Training configuration: tiny SeZM (`channels=16`, two blocks, fp32), 500 Adam steps on `examples/water/data/data_{0..3}`. |
...
Freeze the checkpoint (the pt backend detects SeZM and writes a `.pt2`
